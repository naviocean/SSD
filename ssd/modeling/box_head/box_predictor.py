...
class BoxPredictor(nn.Module):
    ...
    def forward(self, features):
        ...
        cls_logits = torch.cat([c.view(c.shape[0], -1) for c in cls_logits],
                               dim=1).view(batch_size, -1, (len(self.cfg.MODEL.CLASSES)+1))
...
class SSDBoxPredictor(BoxPredictor):
    def cls_block(self, level, out_channels, boxes_per_location):
        return nn.Conv2d(out_channels, boxes_per_location * (len(self.cfg.MODEL.CLASSES)+1), kernel_size=3, stride=1, padding=1)
...
class SSDLiteBoxPredictor(BoxPredictor):
    def cls_block(self, level, out_channels, boxes_per_location):
        ...
        if level == num_levels - 1:
            return nn.Conv2d(out_channels, boxes_per_location * (len(self.cfg.MODEL.CLASSES)+1), kernel_size=1)
        return SeparableConv2d(out_channels, boxes_per_location * (len(self.cfg.MODEL.CLASSES)+1), kernel_size=3, stride=1, padding=1)
